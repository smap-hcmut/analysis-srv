...
Tests that analysis-srv correctly identifies and processes the production
ingest wire format, and rejects unknown formats without emitting Kafka output.

Covered scenarios:
  1. ingest-srv flat format (has `identity` block) → output published
  2. unknown format        (no `identity` block) → no output, error
...
async def test_unknown_format_produces_no_output(harness: E2ETestHarness):
    """Unknown format (no `identity`) → pipeline error, no Kafka output.
